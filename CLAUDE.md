...
2. **Flow** — `computeFlowVariant(answers)`; clicking a stage opens a detail panel with the stage's real status pipeline from the snapshot (`lib/flow/stageDetail.ts` keyword-matches flow nodes to job categories, applies renames, null-safe when snapshot pending) plus notifications; Lead Qualification also shows the answer/no-answer branch story: lead sources fan into Lead/Customer created → optional HubSpot or Lead Qualification → Inspection → optional Insurance claim → CPQ (measurement providers feed in) → Proposal → Material ordering (suppliers fan out) → Production → Job complete → Invoicing & closeout. Zuper Connect parallel. Notification badges per node from `lib/notifications/derive.ts`.
